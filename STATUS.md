...
Модуль **M1 закрыт** (тег `m1`, приёмка по `docs/tasks/M1.md`). Следующий — **M2 бэкенд-фундамент**: отдельной
сессией составить устав `docs/tasks/M2.md` и этапы из `docs/drafts/B0.2`–`B0.6` по итогам этого файла (контракт
`director.json` → хранилище и `project.json` → очередь джобов и SSE → провайдеры и учёт расходов → typegen
из Pydantic, заменяющий `src/types/fixtures.ts`). Хвосты M0 (`tests/`, ruff/mypy, лишний `src/studio/`) — туда же.
